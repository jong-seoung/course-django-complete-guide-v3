...
class BooleanYNField(models.BooleanField):
    ...
    def from_db_value(self, value: Optional[str],
                      expression: Expression, connection: BaseDatabaseWrapper) -> Optional[bool]:
...
class IPv4AddressIntegerField(models.CharField):
    ...
    def db_type(self, connection):
        if connection.vendor == "postgresql": return "bigint"  # 8바이트 정수형
        if connection.vendor == "oracle": return "number(19)"  # 8바이트 정수형
    ...
    def from_db_value(self, value: Optional[int], expression: Expression, connection: BaseDatabaseWrapper) -> Optional[str]:
